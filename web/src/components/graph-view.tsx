...
export interface GraphViewProps { modules: ModuleGraph[]; state?: "populated" | SurfaceState; error?: unknown }
...
export function GraphView({ modules, state = "populated", error }: GraphViewProps) {
  ...
  const onPointerMove = (event: PointerEvent) => {
    ...
    setViewport((value) => ({ ...value, x: value.x + event.clientX - activeDrag.x, y: value.y + event.clientY - activeDrag.y }));
  ...
  return (
    <div class="graph-view" data-visual-fixture="graph-view">
      ...
      <div class="graph-view__canvas">
        <svg viewBox={`0 0 ${String(WIDTH)} ${String(HEIGHT)}`} role="img" aria-label={STRINGS.routeDescriptions.graph}
          ...
          <g transform={`translate(${String(viewport.x)} ${String(viewport.y)}) scale(${String(viewport.scale)})`}>
            ...
            <g>{points.map((point) => <g class="graph-node" transform={`translate(${String(point.x)} ${String(point.y)})`} key={point.id}>
