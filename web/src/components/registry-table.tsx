    ...
    <div class="registry-table__scroller" onScroll={(event) => { if (virtual) setScrollTop(event.currentTarget.scrollTop); }}>
      <table class="registry-table">
        ...
        <tbody>
          ...
          {visible.map((row) => <tr class="registry-table__row" key={`${row.project}:${row.id}`}><td data-label={STRINGS.components.registryTable.columns.brick} title={row.id}>{row.id}</td><td data-label={STRINGS.components.registryTable.columns.project} title={row.project}>{row.project}</td><td data-label={STRINGS.components.registryTable.columns.status}><SealChip status={row.health_status === "ok" ? "pass" : "fail"} label={row.status.toUpperCase()} /></td><td class="registry-table__score" data-label={STRINGS.components.registryTable.columns.score}>{row.score.toLocaleString()}</td></tr>)}
