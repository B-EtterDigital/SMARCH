 ...
 * .smarch/.
 *
 *   node tools/sma-clone.mjs --brick <id> --target /path/to/project
 *   node tools/sma-clone.mjs --build <id> --target /path/to/project
 *   node tools/sma-clone.mjs --build-manifest examples/build.sweetspot.json --target /path/to/project
 *   node tools/sma-clone.mjs --brick <id> --target /path/to/project --write
...
function uniqStrings(values) {
  return [...new Set(values.flatMap((value) => toStringArray(value)).map((value) => value.trim()).filter(Boolean))];
}
  ...
  if (opts.build || opts.buildManifest) {
    ...
    if (opts.write) {
      ...
      smarchImports.imports.push({
        ...
        imported_at: now,
        imported_by: "tools/sma-clone.mjs",
        source_registry: {
          path: opts.registry,
          sha256: registrySha
        },
...
`;
  ...
  const plan = { brick: brick.id, name: brick.name, from_project: brick.project, status: brick.status, actions: [] };
