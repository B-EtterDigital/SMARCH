 ...
 * Usage: `node tools/sma-codex-all.ts --only filter --limit 1`
 */
/**
 ...
 *
 * Usage:
 *   node tools/sma-codex-all.ts                                # everything
 *   node tools/sma-codex-all.ts --project acme-desktop --limit 30       # smoke-test
 *   node tools/sma-codex-all.ts --filter workos                # just auth group
 */
...
function parseArgs(argv: string[]) {
  ...
  const opts: { skip: Set<string>; only: Set<string> | null } = { skip: new Set(), only: null };
...
async function main() {
  ...
  const steps = [
    ...
    { id: "enrich",    cmd: "node", args: [`${tools}/sma-codex-enrich.ts`, ...passthrough] },
    { id: "connect",   cmd: "node", args: [`${tools}/sma-codex-connect.ts`, ...passthrough] },
    { id: "tests",     cmd: "node", args: [`${tools}/sma-codex-test.ts`, ...passthrough] },
    { id: "promote",   cmd: "node", args: [`${tools}/sma-promote.mjs`] },
    { id: "wiki",      cmd: "node", args: [`${tools}/sma-codex-wiki.ts`, ...passthrough] },
    { id: "wiki-idx",  cmd: "node", args: [`${tools}/sma-codex-wiki-index.ts`] }
  ];
