 ...
 *   node tools/sma-license-gate.mjs --gate         # fail on block-severity
 *   node tools/sma-license-gate.mjs --strict       # treat theft-risk as block
...
function collectComponentIds(manifest) {
  const ids = new Set();
...
function hasAttribution(publishing) {
...
function loadLedger() {
  if (!existsSync(LICENSE_LEDGER)) {
    throw new Error(`license ledger not found: ${relative(SMA_ROOT, LICENSE_LEDGER)}. Run: node tools/sma-provenance-ledger.mjs`);
  }
...
function collectBuildManifests(dir) {
  const out = [];
  const stack = [dir];
  while (stack.length) {
    const cur = stack.pop();
