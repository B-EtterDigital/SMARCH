 ...
 * Example: `node tools/sma-portfolio-refresh.ts --help`
 */
/**
 * sma-portfolio-refresh.ts — queued/debounced Gen3 portfolio refresh.
 *
...
function usage() {
  console.log(`Usage:
  sma-portfolio-refresh.ts [--force] [--json]
...
async function refreshPortfolio(): Promise<RefreshSummary> {
  ...
  if (!args.noProvenance) {
    const provenancePhase = await maybeRefresh({
      ...
      command: [
        process.execPath,
        resolve(toolsDir, 'sma-provenance-ledger.ts'),
    ...
    summary.phases.push(await maybeRefresh({
      ...
      command: [
        process.execPath,
        resolve(toolsDir, 'sma-anchor.ts'),
      ],
    }));
  }
  ...
  if (!args.noDashboard) {
    summary.phases.push(await maybeRefresh({
      ...
      command: [
        process.execPath,
        resolve(toolsDir, 'sma-gen3-dashboard.ts'),
        'build',
      ],
    }));
  }
...
function scanPhaseConfig(projectIds: string[], impact: ChangedImpact, changedFiles: string[]): RefreshPhase {
  if (projectIds.length > 0) {
    ...
    return {
      ...
      command: [
        process.execPath,
        resolve(toolsDir, 'sma-portfolio-scan.ts'),
        '--all',
        '--no-refresh',
...
async function waitForLeaseClear(phase: RefreshPhase, active: ActiveLease): Promise<void> {
  ...
  const err = new Error(`${phase.name}: timed out waiting for ${active.resource_kind}:${active.resource_id} (${active.lease_id})`) as Error & { code?: number };
  err.code = 10;
  throw err;
}
