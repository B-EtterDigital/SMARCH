...
function usage() {
  console.log(`Usage:
  sma-dirty-baseline.mjs save  --project <id>|--root <path> [--label <name>] [--json]
  sma-dirty-baseline.mjs delta --project <id>|--root <path> [--baseline <id>] [--label <name>] [--limit <n>] [--json]
  sma-dirty-baseline.mjs list  --project <id>|--root <path> [--label <name>] [--json]
  sma-dirty-baseline.mjs clean --project <id>|--root <path> [--keep <n>]
  sma-dirty-baseline.mjs path
...
function buildBaseline(project, overrides = {}) {
...
function diffStatuses(baseline, current) {
  ...
  const after = new Map(current.records.map((record) => [record.path, record]));
  const added = [];
  const cleared = [];
  const changed = [];
