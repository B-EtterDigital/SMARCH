...
function main() {
  ...
  const rootMap = new Map<string, string>((registry.scanned_project_roots || []).map((r) => [r.id, r.root]));

  const signer = args.sign ? loadSigner() : null;
  const licenseCache = new Map<string, any>();
...
function gitTrail(abs: string, cache: Map<string, string | null>) {
...
function touchEvent(commit: any, role: string, summary: string) {
  return {
    actor_kind: 'human',
...
function resolveLicense(brick: any, projectAbs: string | null, cache: Map<string, any>, { evidence = false, absPath = null }: { evidence?: boolean; absPath?: string | null } = {}) {
...
function brickDeclaredLicense(brick: any): string | null {
...
function projectLicense(projectAbs: string, cache: Map<string, any>) {
...
function detectCollisions(byHash: Map<string, any[]>) {
  const collisions = [];
...
function annotateCopies(fingerprints: any[], collisions: any[]): void {
  const map = new Map();
...
function writeLedgers({ header, fingerprints, provenance, licenses, collisions, resolvedCount, gitCount }: Record<string, any>): void {
...
function printSummary(s: any, collisions: any[]): void {
...
function guessProjectRoot(brick: any): string | null {
