...
function usage() {
  console.log(`Usage:
  sma-context-check.mjs check  --project <id> [--since <iso|ref>] [--max-age-minutes <n>]
                               [--strict] [--json]
  sma-context-check.mjs audit  --project <id> [--json]
...
function runCheck() {
  ...
  if (args.json) {
    ...
    if (missing.length) {
      ...
      for (const r of missing) {
        if (!r.brick) continue;
        console.log(`  node tools/sma-context.mjs append --project ${args.project} --brick ${r.brick} --kind edit_applied --intent "<what you did>" --decision "<why>"`);
      }
    }
  }
