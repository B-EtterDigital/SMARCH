...
function uniqStrings(values): string[] {
  return [...new Set<string>((values || []).filter((value) => typeof value === "string" && value.trim()).map((value) => value.trim()))];
}
...
function normalizeActorRecord(input, fallbackTimestamp) {
  ...
  const record: Record<string, any> = { actor };
...
function buildExternalPackages(manifest, artifactType) {
  ...
  const packages = new Map<string, Record<string, any>>();
  ...
  return [...packages.values()].map((entry) => {
    const next: Record<string, any> = { name: entry.name };
...
function buildSourceChain({ manifest, manifestPathRelative, artifactType, artifactId, manifestHash, sourceCommit, registryRef, registryHash }) {
  const chain: Array<{ kind: string; ref: any; hash?: any }> = [
    {
      kind: "manifest",
