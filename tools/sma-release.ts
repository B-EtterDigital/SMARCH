 ...
 * Example: `node tools/sma-release.ts --help`
 */
...
interface ReleaseArgs {
  searchRoots: string[]; supersedes: string[]; stdout: boolean; dryRun: boolean;
  help?: boolean; breaking?: boolean; allowClosed?: boolean; manifest?: string; out?: string; channel?: string; status?: string; version?: string; createdAt?: string; publishedAt?: string; sourceCommit?: string; registry?: string; registrySha?: string; previousRelease?: string; replacementRelease?: string; note?: string; registryOrigin?: string;
}
interface ArtifactRef { brick_id?: string; required?: boolean }
interface ActorInput { actor?: unknown; actor_kind?: unknown; actor_id?: unknown; primary?: unknown; tool?: unknown; provider?: unknown; method?: unknown; model?: unknown; timestamp?: unknown }
interface CheckInput { name?: string; command?: string; status?: string; evidence_path?: string }
interface EnvEntry { name?: string; required?: boolean; optional?: boolean; optional_in?: unknown[]; forbidden_in?: string[] }
interface ReleaseManifest {
  build?: { id?: string; version?: string; status?: string; runtimes?: string[] };
  brick?: { id?: string; version?: string; status?: string; language?: string | string[]; frameworks?: string[] };
  source?: { paths?: string[]; supporting_artifacts?: string[]; derived_from_bricks?: ArtifactRef[]; project?: string; commit?: string };
  composition?: { brick_refs?: ArtifactRef[]; optional_bricks?: ArtifactRef[] };
  interfaces?: { entrypoints?: string[]; api_endpoints?: string[]; commands?: string[]; public_api?: string[]; events?: string[]; ui_surfaces?: string[]; required_dependencies?: string[] };
  verification?: { status?: string; evidence?: CheckInput[]; fixture_targets?: string[]; smoke_commands?: string[]; integration_targets?: string[] };
  clone?: { readiness?: string; target_docs?: string[]; post_clone_checks?: string[]; rollback_steps?: string[]; adaptation_points?: string[] };
  supply_chain?: { dependencies?: { name?: string; version?: string; version_range?: string }[] };
  contracts?: { env?: { required?: EnvEntry[]; optional?: EnvEntry[]; forbidden?: string[] }; data?: unknown; rls?: unknown };
  security?: { env?: { required?: boolean; variables?: EnvEntry[] }; rls?: unknown };
  classification?: { data_classes?: string[] };
  quality?: { verification?: CheckInput[]; test_commands?: string[] };
  upgrade?: { migration_hooks?: string[]; rollback_supported?: boolean };
  provenance?: { created_by?: ActorInput; reviewed_by?: ActorInput[] };
  publishing?: { visibility?: 'private' | 'internal' | 'community' | 'public' };
}
interface ContractSections { dependency_refs: { artifact_type: string; artifact_id: string; required?: boolean }[]; external_packages: Record<string, unknown>[] }
interface SourceChainInput { manifest: ReleaseManifest; manifestPathRelative: string; artifactType: ArtifactType; artifactId: string; manifestHash: string; sourceCommit: string; registryRef: string | null; registryHash: string }
interface ContentBuildInput { manifest: ReleaseManifest; artifactType: ArtifactType; manifestPath: string; searchRoots: string[]; cwd: string }
const HELP_TEXT = `Usage: node tools/sma-release.ts --manifest <path> [options]
...
function uniqStrings(values: readonly unknown[] | null | undefined): string[] {
  return [...new Set((values || []).filter((value): value is string => typeof value === "string" && Boolean(value.trim())).map((value) => value.trim()))];
}

function semverOrFallback(value: unknown, fallback = "0.1.0"): string {
...
function toPosixRelative(fromRoot: string, targetPath: string): string {
  return path.relative(fromRoot, targetPath).split(path.sep).join("/");
}

function readJsonFile(filePath: string): Promise<ReleaseManifest> {
  return fs.readFile(filePath, "utf8").then((text) => JSON.parse(text) as ReleaseManifest);
}
...
function inferArtifactType(manifest: ReleaseManifest): ArtifactType {
...
function inferArtifactId(manifest: ReleaseManifest, artifactType: ArtifactType): string {
...
function inferVersion(manifest: ReleaseManifest, artifactType: ArtifactType, overrideVersion?: string): string {
...
function inferChannel({ manifest, artifactType, version, overrideChannel }: { manifest: ReleaseManifest; artifactType: ArtifactType; version: string; overrideChannel?: string }): string {
...
function inferReleaseStatus({ manifest, artifactType, overrideStatus }: { manifest: ReleaseManifest; artifactType: ArtifactType; overrideStatus?: string }): string {
...
function inferVerificationStatus(manifest: ReleaseManifest, artifactType: ArtifactType): string {
...
function normalizeActorRecord(input: ActorInput | null | undefined, fallbackTimestamp: string): Record<string, unknown> | null {
  ...
  const record: Record<string, unknown> = { actor };
...
function detectProjectsRoot(cwd: string): string | null {
  const direct = path.resolve(cwd, "..", "Projects");
  if (existsSync(direct)) return direct;
  return null;
}

async function listChildDirectories(rootPath: string | null): Promise<string[]> {
...
async function resolveDeclaredPath(declaredPath: string, searchRoots: readonly string[]): Promise<string | null> {
...
function classifyArtifactKind(targetPath: string, stat: Stats): string {
...
async function contentDigestForPath(absolutePath: string): Promise<{ sha256: string; executable: boolean }> {
...
function manifestPortableDocs(manifest: ReleaseManifest, artifactType: ArtifactType): string[] {
...
function manifestEntrypoints(manifest: ReleaseManifest, artifactType: ArtifactType): string[] {
  if (artifactType === "build") {
    return uniqStrings([
...
function buildDependencyRefs(manifest: ReleaseManifest, artifactType: ArtifactType): ContractSections['dependency_refs'] {
  if (artifactType !== "build") return [];
  const merged = new Map<string, { artifact_type: string; artifact_id: string; required?: boolean }>();
  const refs = [
...
function buildExternalPackages(manifest: ReleaseManifest, artifactType: ArtifactType): Record<string, unknown>[] {
  if (artifactType !== "brick") return [];
  const packages = new Map<string, { name: string; version_range?: string; scope?: string }>();
...
function buildEnvContract(manifest: ReleaseManifest, artifactType: ArtifactType): { required: string[]; optional: string[]; forbidden: string[] } {
  if (artifactType === "build") {
  ...
  for (const entry of variables) {
    ...
    const isOptional = !env.required || !entry.required || Array.isArray(entry.optional_in) || entry.optional;
...
function buildContractHashes(manifest: ReleaseManifest, artifactType: ArtifactType, contractSections: ContractSections): Record<string, string> {
...
function inferRuntimes(manifest: ReleaseManifest, artifactType: ArtifactType): string[] {
  if (artifactType === "build") {
  ...
  const languages = uniqStrings(Array.isArray(manifest.brick?.language) ? manifest.brick.language : [manifest.brick?.language]);
...
function buildPublicInterfaces(manifest: ReleaseManifest, artifactType: ArtifactType): string[] {
  if (artifactType === "build") {
    return uniqStrings([
      ...(manifest.interfaces?.entrypoints || []),
      ...(manifest.interfaces?.api_endpoints || []),
...
function buildDataClasses(manifest: ReleaseManifest): string[] {
  return uniqStrings(manifest.classification?.data_classes || []);
}

function mapCheckStatus(value: unknown): string {
...
function buildVerificationChecks(manifest: ReleaseManifest, artifactType: ArtifactType): { name: string; status: string; command?: string; evidence_path?: string }[] {
  if (artifactType === "build") {
...
function buildMigrationSection(manifest: ReleaseManifest, artifactType: ArtifactType): { manual_steps: string[] } | undefined {
...
function buildRollbackSection(manifest: ReleaseManifest, artifactType: ArtifactType): { notes: string; commands: string[] } | undefined {
...
function createdByRecord(manifest: ReleaseManifest, createdAt: string): Record<string, unknown> {
  const actor = normalizeActorRecord(manifest.provenance?.created_by, createdAt);
  if (actor) return actor;
  return {
    actor: process.env.USER || "unknown",
    tool: "tools/sma-release.ts",
    timestamp: createdAt
  };
}

function reviewedByRecords(manifest: ReleaseManifest, createdAt: string): Record<string, unknown>[] {
  return (manifest.provenance?.reviewed_by || [])
    .map((entry) => normalizeActorRecord(entry, createdAt))
    .filter((entry): entry is Record<string, unknown> => entry !== null);
}

function buildSourceChain({ manifest, manifestPathRelative, artifactType, artifactId, manifestHash, sourceCommit, registryRef, registryHash }: SourceChainInput) {
  const chain: { kind: string; ref: string; hash?: string }[] = [
    {
      kind: "manifest",
...
function normalizeDateTime(value: string | undefined, flagName: string): string | undefined {
...
function fallbackHash(label: string, sourceValue: string): string {
  return sha256Text(`${label}:${sourceValue}`).slice(0, 64);
}

function gitCommitForPath(targetPath: string): string | null {
  try {
...
async function resolveRegistryHash(cwd: string, overrideRegistry: string | undefined, overrideHash: string | undefined, manifestPath: string): Promise<{ hash: string; ref: string | null }> {
  if (overrideHash) {
...
async function buildContentSection({ manifest, artifactType, manifestPath, searchRoots, cwd }: ContentBuildInput) {
  ...
  const allEntries: { declaredPath: string; absolutePath: string; artifactKind: string | null }[] = [];
  ...
  const artifactMap = new Map<string, { path: string; kind: string; sha256: string; executable?: boolean }>();
  for (const entry of allEntries) {
    ...
    const artifact: { path: string; kind: string; sha256: string; executable?: boolean } = {
...
function buildContractsSection(manifest: ReleaseManifest, artifactType: ArtifactType): Record<string, unknown> {
...
function buildVerificationSection(manifest: ReleaseManifest, artifactType: ArtifactType, verificationStatus: string): Record<string, unknown> & { status: string; verified_at?: string } {
  const verification: Record<string, unknown> & { status: string; verified_at?: string } = {
...
async function main() {
  ...
  try {
    ...
    if (error instanceof ExportBlockedError) { console.error(error.message); process.exit(3); }
    throw error;
  }
