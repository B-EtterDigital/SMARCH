/* Dashboard diagnostics intentionally preserve JavaScript's existing coercion for opaque registry metadata. */
/* Dashboard renderers are declarative templates; complexity counts nullish display fallbacks as control-flow branches. */
...
export interface RegistryProject { id: string; root: string; security_gate?: { status?: string; high_or_critical?: number }; error_count?: number; warning_count?: number; health_counts?: Record<string, number>; brick_count?: number }
interface ProjectMeta { sma?: { security_gate?: { status?: string; high_or_critical?: number } } }
interface ReadinessMetrics { blocked_clone_count?: number; drift_count?: number; boundary_violation_count?: number; same_group_coupling_count?: number; env_gap_count?: number; compliance_score?: number; unmanifested_count?: number }
interface ComplianceDimension { label?: string; coverage_rate?: number; coverage_units?: number; ready_count?: number; total_count?: number }
interface ComplianceReport { score?: number; grade?: string; trackable_brick_count?: number; weakest_dimensions?: ComplianceDimension[]; dimensions?: Record<string, ComplianceDimension>; highest_gap_bricks?: GapEntry[] }
interface ReadinessProject { project?: string; readiness?: { score?: number; grade?: string; label?: string; reasons?: string[]; metrics?: ReadinessMetrics }; compliance_report?: ComplianceReport }
export interface QueueEntry { severity?: string; rank?: number; project?: string; path?: string; first_action?: string; strategy?: string; theme?: string; lines?: number; expected_slices?: number }
interface BoundaryEntry { kind?: string; project?: string; file?: string; path?: string; specifier?: string; target?: string }
interface RiskEntry { project?: string; name?: string; brick_id?: string; path?: string; effective_status?: string; blocker_codes?: string[]; warning_codes?: string[]; raw_source_tokens?: number }
type GapEntry = RiskEntry & { undeclared_env_refs?: string[]; missing_dimensions?: string[] };
interface BuildCandidate { project?: string; name?: string; candidate_key?: string; recurrence_key?: string; dominant_feature_cluster?: string; dominant_domain?: string; dominant_path_root?: string; dominant_group?: string; confidence_score?: number; confidence_label?: string; brick_count?: number; recurrent_project_count?: number; detection_sources?: string[]; why?: string; sample_paths?: string[] }
interface Finding { rule_id?: string; code?: string; summary?: string; message?: string }
interface CuratedBuild { source_project?: string; name?: string; artifact_id?: string; version?: string; required_brick_ref_count?: number; brick_ref_count?: number; promotion_desired_status?: string; suggested_build_status?: string; status?: string; update_ready?: boolean; release_count?: number; latest_channel?: string; latest_release_status?: string; verification_top_blockers?: Finding[]; private_publish_top_blockers?: Finding[]; promotion_blockers?: Finding[]; private_publish_status?: string; promotion_priority?: string; readiness_score?: number; publishability_score?: number; publish_ready?: boolean }
interface ReleaseArtifact { source_projects?: string[]; artifact_id?: string; latest_release?: { path?: string; version?: string; channel?: string; status?: string; trust_summary?: { trust_level?: string; verification_status?: string; check_counts?: { total?: number } } } }
interface PublishBundle { publish_safe?: boolean; decision?: { status?: string; counts?: { blocker?: number } }; top_blockers?: Finding[]; top_warnings?: Finding[]; artifact?: { type?: string; original_id?: string; community_id?: string }; bundle_path?: string; publishing_visibility?: string }
interface InstallTarget { target_root?: string; selected_build_count?: number; build_ids?: string[]; resolved_brick_count?: number; imports_count?: number; placement_count?: number; update_event_count?: number }
export interface ActionEntry { project?: string; category?: string; name?: string; brick_id?: string; brick_name?: string; path?: string; first_action?: string; why?: string; priority_score?: number; total_matches?: number; smell_score?: number; top_types?: { label?: string; key?: string; count?: number }[] }
interface QualityProject { project?: string; code_quality_report?: { hotspot_file_count?: number; score?: number; grade?: string; total_smell_count?: number; duplicate_cluster_count?: number }; remediation_counts?: { quality?: number }; canonicalization?: { top_targets?: CanonicalTarget[]; bottleneck_stage?: string } }
interface DuplicateCluster { stem?: string; count: number; projects: string[]; bricks: { project: string; name?: string; id?: string }[] }
interface TokenEntry { project?: string; name?: string; brick_id?: string; path?: string; raw_source_tokens?: number; summary_tokens?: number; estimated_summary_tokens?: number; estimated_savings_tokens?: number; file_count?: number }
interface CanonicalTarget { project?: string; target_type?: string; name?: string; target_id?: string; priority_score?: number; promotion_stage?: string; confidence_label?: string; blocker_reasons?: string[]; evidence_summary?: { brick_count?: number; duplicate_count?: number; why?: string }; blocker_summary?: Record<string, number> }
export interface CanonicalizationView { top_targets?: CanonicalTarget[]; reasons?: { code?: string; message?: string; current?: number; threshold?: number }[]; counts?: { build_target_count?: number; brick_target_count?: number; ready_project_count?: number; project_work_bottleneck_count?: number; artifact_promotion_bottleneck_count?: number; project_count?: number }; project_canonicalization_ready?: boolean }
interface QualityReportView { average_score?: number; score?: number; average_grade?: string; grade?: string; hotspot_file_count?: number; total_smell_count?: number; duplicate_cluster_count?: number }
interface PublishSummary { bundle_count?: number; publish_safe_count?: number; blocked_count?: number; finding_count?: number }
export interface StateSnapshot { build_plane?: { curated_builds?: CuratedBuild[]; released_curated_build_count?: number; curated_manifest_count?: number; verification_ready_count?: number; publish_ready_count?: number; update_ready_build_count?: number; average_publishability_score?: number; promotion_ready_count?: number; installable_build_count?: number; rollback_supported_build_count?: number; candidate_or_better_verification_count?: number; private_publish_bundle_count?: number; private_publish_safe_count?: number }; release_plane?: { top_build_releases?: ReleaseArtifact[]; summary?: { build?: { artifact_count?: number; published_artifact_count?: number; channels?: Record<string, number>; stable_or_lts_artifact_count?: number }; release_count?: number } }; publish_plane?: { bundles?: PublishBundle[]; summary?: PublishSummary }; install_plane?: { targets?: InstallTarget[]; target_count?: number; update_event_count?: number; selected_build_count?: number; resolved_brick_count?: number; import_count?: number; placement_count?: number; latest_event_at?: string; scan_roots?: string[] }; projects?: QualityProject[]; trust?: { build_candidates?: BuildCandidate[]; canonicalization?: CanonicalizationView; quality_queue?: ActionEntry[]; code_quality_report?: QualityReportView; readiness?: { average_score?: number; average_grade?: string }; compliance?: { average_score?: number; average_grade?: string }; remediation_counts?: { env_contract?: number; rls_contract?: number; boundary?: number; quality?: number } }; totals?: PortfolioTotals; promotion_plane?: { summary?: { auto_promotable_count?: number; build_count?: number } } }
export interface ScannerView { readiness?: { projects?: ReadinessProject[]; average_score?: number; average_grade?: string }; boundary_report?: { top_violations?: BoundaryEntry[]; private_cross_brick_import_count?: number; cross_brick_owned_import_count?: number; same_group_internal_import_count?: number; unresolved_local_import_count?: number }; clone_preflight?: { highest_risk_bricks?: RiskEntry[]; counts?: Record<string, number> }; env_contract_report?: { highest_gap_bricks?: GapEntry[]; bricks_with_undeclared_refs?: number; undeclared_reference_count?: number; ignored_reference_count?: number }; compliance_report?: ComplianceReport & { average_score?: number; average_grade?: string }; build_report?: { top_candidates?: BuildCandidate[]; candidate_signatures?: BuildCandidate[]; average_confidence_score?: number; candidate_count?: number; recurrent_family_count?: number; recurrent_candidate_count?: number; detected_brick_count?: number }; remediation_report?: { top_actions?: ActionEntry[]; project_action_plans?: { project?: string; actions?: ActionEntry[] }[]; quality_queue?: ActionEntry[]; counts?: { env_contract?: number; rls_contract?: number; boundary?: number; quality?: number } }; duplicate_clusters?: DuplicateCluster[]; token_economics?: { top_token_heavy_bricks?: TokenEntry[]; raw_source_tokens?: number; estimated_summary_tokens?: number }; canonicalization?: CanonicalizationView; code_quality_report?: QualityReportView; manifest_drift?: { count?: number } }
interface CapabilityFamily { key: string; label: string; feature: string; domain: string; projects: Set<string>; occurrence_count: number; total_brick_count: number; max_confidence_score: number; confidence_total: number; detection_sources: Set<string>; examples: { name: string; project: string; brick_count: number; confidence_score: number; why: string; path: string }[] }
interface PortfolioTotals { project_count?: number; brick_count?: number; status_counts?: Record<string, number> }
interface SurfaceMetric { label: string; value: unknown; note?: string }
...
export async function projectMetadata(projects: RegistryProject[]): Promise<Map<string, ProjectMeta>> {
  ...
  for (const project of projects) {
    ...
    try {
      ...
      if (isRecord(meta)) byId.set(project.id, meta);
    } catch {
  ...
  return projects.map((entry) => {
    ...
    return `      <article class="scanner-brick scanner-brick--${tone}">
        ...
        <div class="scanner-brick-head">
          ...
          <strong>${String(readiness.score ?? 0)}<small>/${escapeHtml(readiness.grade ?? "F")}</small></strong>
        </div>
        <h3>${escapeHtml(readiness.label ?? "unknown")}</h3>
        <dl>
          <div><dt>Blocked clone</dt><dd>${String(metrics.blocked_clone_count ?? 0)}</dd></div>
          <div><dt>Drift</dt><dd>${String(metrics.drift_count ?? 0)}</dd></div>
          <div><dt>Boundary hits</dt><dd>${String(metrics.boundary_violation_count ?? 0)}</dd></div>
          <div><dt>Coupling</dt><dd>${String(metrics.same_group_coupling_count ?? 0)}</dd></div>
          <div><dt>Env gaps</dt><dd>${String(metrics.env_gap_count ?? 0)}</dd></div>
          <div><dt>Compliance</dt><dd>${String((compliance.score ?? metrics.compliance_score) ?? 0)}/${escapeHtml(compliance.grade ?? "F")}</dd></div>
          <div><dt>Unmanifested</dt><dd>${String(metrics.unmanifested_count ?? 0)}</dd></div>
        </dl>
...
export function scannerQueueCards(queue: QueueEntry[] | null | undefined): string {
  return (queue ?? []).slice(0, 12).map((entry) => `      <article class="queue-card queue-card--${escapeHtml(entry.severity ?? "medium")}">
        <div class="queue-rank">#${String(entry.rank)}</div>
        ...
        <dl>
          ...
          <div><dt>Slices</dt><dd>${String(entry.expected_slices ?? 0)}</dd></div>
...
export function cloneRiskCards(scanner: ScannerView | null | undefined): string {
  ...
  return rows.slice(0, 10).map((entry) => `      <article class="risk-card risk-card--${escapeHtml(entry.effective_status ?? "manual_review")}">
        ...
        <dl>
          ...
          <div><dt>Blockers</dt><dd>${String((entry.blocker_codes ?? []).length)}</dd></div>
          <div><dt>Warnings</dt><dd>${String((entry.warning_codes ?? []).length)}</dd></div>
...
export function complianceProjectCards(scanner: ScannerView | null | undefined): string {
  ...
  return rows.slice(0, 8).map((entry) => {
    ...
    const weakest = (compliance.weakest_dimensions ?? []).slice(0, 2).map((dimension) => `${String(dimension.label)} ${String(dimension.coverage_rate)}%`).join(" · ");

    return `      <article class="gap-card gap-card--${tone}">
        <p>${escapeHtml(entry.project ?? "")}</p>
        <h3>${String(compliance.score ?? 0)}<small>/${escapeHtml(compliance.grade ?? "F")}</small></h3>
...
export function complianceDimensionRows(scanner: ScannerView | null | undefined): string {
  const dimensions = (Object.entries(scanner?.compliance_report?.dimensions ?? {}) as [string, LooseRecord][])
    .filter(([, dimension]) => Number(dimension.total_count ?? 0) > 0)
    .sort((a, b) => Number(a[1].coverage_rate ?? 0) - Number(b[1].coverage_rate ?? 0) || a[0].localeCompare(b[0]));
  ...
  return dimensions.slice(0, 9).map(([, dimension]) => `        <li>
          ...
          <div class="compliance-bar"><b style="width:${String(Math.max(6, Number(dimension.coverage_rate ?? 0)))}%"></b></div>
          <em>${String(dimension.coverage_rate ?? 0)}%</em>
        </li>`).join("\n");
}
...
export function complianceGapCards(scanner: ScannerView | null | undefined): string {
  ...
  return rows.slice(0, 10).map((entry) => `      <article class="gap-card gap-card--${escapeHtml(entry.effective_status ?? "manual_review")}">
        ...
        <h3>${escapeHtml((entry.name ?? entry.brick_id) ?? "brick")}</h3>
        <code>${escapeHtml(entry.path ?? "")}</code>
        <ul>${(entry.missing_dimensions ?? []).slice(0, 4).map((dimension) => `<li>${escapeHtml(dimension.replaceAll("_", " "))}</li>`).join("")}</ul>
      </article>`).join("\n");
}
...
export function buildCandidateCards(scanner: ScannerView | null | undefined, limit = 8): string {
  ...
  return rows.slice(0, limit).map((entry) => {
    const tone = (entry.confidence_score ?? 0) >= 90 ? "ready" : (entry.confidence_score ?? 0) >= 75 ? "review" : "danger";
    const sources = (entry.detection_sources ?? []).slice(0, 4).join(" · ");
    const recurrence = (entry.recurrent_project_count ?? 0);
    ...
    return `      <article class="build-card build-card--${tone}">
        ...
        <dl>
          <div><dt>Confidence</dt><dd>${String(entry.confidence_score ?? 0)}/${escapeHtml(entry.confidence_label ?? "unknown")}</dd></div>
...
function buildVerificationTone(entry: CuratedBuild | null | undefined): string {
  const suggested = (entry?.suggested_build_status ?? "").toLowerCase();
  if (suggested === "canonical" || entry?.publish_ready) return "ready";
  if (suggested === "verified" || (entry?.readiness_score ?? 0) >= 75) return "review";
  return "danger";
}
...
export function curatedBuildCards(stateSnapshot: StateSnapshot | null | undefined, limit = 8): string {
  ...
  return rows.slice(0, limit).map((entry) => {
    ...
    return `      <article class="build-card build-card--${tone}">
        ...
        ${blocker ? `<p>${escapeHtml(`${String(blocker.rule_id ?? blocker.code)}: ${(blocker.summary ?? blocker.message) ?? "Finding recorded."}`)}</p>` : ""}
      </article>`;
  }).join("\n");
}
...
export function remediationActionCards(scanner: ScannerView | null | undefined): string {
  ...
  return rows.slice(0, 12).map((entry) => `      <article class="action-card action-card--${escapeHtml(entry.category ?? "boundary")}">
        ...
        <code>${escapeHtml(entry.path ?? "")}</code>
        <span class="action-tag">${escapeHtml((entry.category ?? "action").replaceAll("_", " "))}</span>
...
export function qualityQueueCards(rows: ActionEntry[] | null | undefined, limit = 8): string {
  return toArray(rows).slice(0, limit).map((entry) => `      <article class="plan-card">
        ...
        <ul>
          ...
          <li>dominant: ${escapeHtml(toArray(entry.top_types).slice(0, 2).map((item) => `${String(item.label ?? item.key)} x${formatNumber(item.count ?? 0)}`).join(" · ") || "none recorded")}</li>
        </ul>
      </article>`).join("\n");
}
...
export function qualityProjectCards(stateSnapshot: StateSnapshot | null | undefined, limit = 8): string {
  return toArray(stateSnapshot?.projects)
    .filter((entry) => (entry.code_quality_report?.hotspot_file_count ?? 0) > 0)
    .sort((left, right) =>
      (left.code_quality_report?.score ?? 100) - (right.code_quality_report?.score ?? 100)
      || (right.code_quality_report?.hotspot_file_count ?? 0) - (left.code_quality_report?.hotspot_file_count ?? 0)
...
export function duplicateCards(scanner: ScannerView | null | undefined): string {
  ...
  return clusters.slice(0, 10).map((cluster) => `      <article class="duplicate-card">
        ...
        <h3>${String(cluster.count)} overlap${cluster.count === 1 ? "" : "s"}</h3>
        <span>${String(cluster.projects.length)} project${cluster.projects.length === 1 ? "" : "s"}</span>
...
export function tokenCards(scanner: ScannerView | null | undefined): string {
  ...
  return rows.slice(0, 8).map((entry) => {
    const raw = (entry.raw_source_tokens ?? 0);
    const summary = ((entry.summary_tokens ?? entry.estimated_summary_tokens) ?? 0);
    ...
    return `      <article class="token-card">
        ...
        <h3>${escapeHtml((entry.name ?? entry.brick_id) ?? "brick")}</h3>
        <code>${escapeHtml(entry.path ?? "")}</code>
        <div class="token-bar"><b style="width:${String(ratio)}%"></b></div>
        <dl>
          ...
          <div><dt>Files</dt><dd>${String(entry.file_count ?? 0)}</dd></div>
        </dl>
      </article>`;
  }).join("\n");
}
...
export function capabilityFamilies(stateSnapshot: StateSnapshot | null | undefined, scanner: ScannerView | null | undefined) {
  ...
  for (const entry of buildCandidateSource(stateSnapshot, scanner)) {
    // Empty identifiers must continue falling through to the next stable identity candidate.
    ...
    current.total_brick_count += (entry.brick_count ?? 0);
    current.max_confidence_score = Math.max(current.max_confidence_score, (entry.confidence_score ?? 0));
    current.confidence_total += (entry.confidence_score ?? 0);
    ...
    if (current.examples.length < 4) {
      current.examples.push({
        ...
        brick_count: (entry.brick_count ?? 0),
...
function topSummaryItems(summary: Record<string, number> | null | undefined, limit = 3): [string, number][] {
...
function canonicalTargetTone(entry: CanonicalTarget | null | undefined): string {
  ...
  if (entry?.promotion_stage === "stabilize_then_promote" || (entry?.priority_score ?? 0) >= 150) {
    return "review";
  }
  return "danger";
}
  ...
  return capabilityFamilies(stateSnapshot, scanner).slice(0, limit).map((entry) => {
    ...
    return `      <article class="build-card build-card--${tone}">
        ...
        <dl>
          ...
          <div><dt>Confidence</dt><dd>${String(entry.average_confidence_score)}/100</dd></div>
        </dl>
...
export function proofSurfaceCards(stateSnapshot: StateSnapshot | null | undefined, scanner: ScannerView | null | undefined, totals: PortfolioTotals, projectCount: number): string {
  ...
  const cards = [
    ...
    {
      tone: (scanner?.build_report?.average_confidence_score ?? 0) >= 80 ? "ready" : "review",
    ...
    {
      tone: (buildPlane.released_curated_build_count ?? 0) > 0 ? "review" : "danger",
    ...
    {
      tone: (canonicalization.counts?.build_target_count ?? 0) > 0 || (canonicalization.counts?.brick_target_count ?? 0) > 0 ? "review" : "steady",
