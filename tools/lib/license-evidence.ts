 ...
 * @example node --input-type=module -e "import { scanText } from './tools/lib/license-evidence.ts'; console.log(scanText('// SPDX-License-Identifier: MIT'))"
 */
/**
 * license-evidence.ts — scan source files for LICENSE EVIDENCE so a brick's
...
export function scanDirectory(absDir: string, { maxFiles = 2000 }: { maxFiles?: number } = {}): DirectoryEvidence {
  ...
  const walk = (dir: string, depth: number): void => {
    ...
    try {
      ...
      console.error(JSON.stringify({ area: 'license-evidence.walk', severity: 'warning', hint: 'Check the source directory and its permissions.', error: error instanceof Error ? error.message : String(error) }));
      return;
    }
...
function readHead(filePath: string, maxBytes = MAX_READ_BYTES): string | null {
  ...
  try {
    ...
    if (code !== 'ENOENT') console.error(JSON.stringify({ area: 'license-evidence.open', severity: 'warning', hint: 'Check the evidence file and its permissions.', error: error instanceof Error ? error.message : String(error), ...(code ? { code } : {}) }));
    return null;
  }
  try {
    ...
    console.error(JSON.stringify({ area: 'license-evidence.read', severity: 'warning', hint: 'Check the evidence file for read errors.', error: error instanceof Error ? error.message : String(error) }));
    return null;
  } finally {
    try { closeSync(fd); } catch (error) {
      console.error(JSON.stringify({ area: 'license-evidence.close', severity: 'warning', hint: 'Inspect the file descriptor lifecycle.', error: error instanceof Error ? error.message : String(error) }));
    }
  }
}
