...
export async function loadRegistry(smaRoot, input) {
  ...
  const projects = (Array.isArray(source.projects) ? source.projects : []).slice(0, DASH_API_MAX_ROWS).map((project) => ({ id: String(project.id || "unknown"), brick_count: number(project.brick_count), average_score: number(project.average_score) }));
  const allBricks = (Array.isArray(source.bricks) ? source.bricks : []).map((brick) => ({ id: String(brick.id || "unknown"), project: String(brick.project || "unknown"), status: String(brick.status || "candidate"), score: number(brick.score), health_status: String(brick.health?.status || "unknown") }));
