...
async function readConflictEvents(directory) {
  ...
  const files = entries.filter((entry) => entry.isFile() && entry.name.endsWith(".ndjson")).sort((a, b) => a.name.localeCompare(b.name));
...
export function handleConflicts(options) {
  return runReadHandler({ ...options, area: "dashboard.api.conflicts", scope: CONFLICTS_SCOPE, validate: validateConflictsQuery, load: options.load || ((input) => loadConflicts(options.root, input)) });
}
