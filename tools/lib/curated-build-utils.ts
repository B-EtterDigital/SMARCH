/* Curated-build reports intentionally preserve JavaScript's existing coercion for opaque external metadata. */
/* Curated-build assembly is a declarative evidence join; complexity counts independent optional fields as branches. */
...
interface BuildManifest { build?: { id?: string }; source?: { project?: string; paths?: string[] } }
interface ManifestEntry { absolutePath: string; relativePath: string; buildId: string | null; manifest: BuildManifest | null }
interface ProjectEntry { id?: string; project?: string; root?: string }
interface PromotionEntry { build_id?: string; blockers?: Blocker[] }
interface VerificationEntry { build_id?: string }
interface PublishReport { findings?: PublishFinding[] }
...
interface StateBuild { artifact_id?: string; source_project?: string; manifest_path?: string; name?: string; [key: string]: unknown }
interface StateSnapshot { build_plane?: { curated_builds?: StateBuild[] }; projects?: { project?: string }[] }
interface AliasEntry { actual_path: string; absolute_path: string; root_relative_path: string }
...
function resolveCuratedBuildPaths(options: Record<string, string | undefined>) {
  return {
...
export function uniqueStrings(values: readonly unknown[] | null | undefined): string[] {
  ...
  for (const value of values ?? []) {
    ...
    for (const entry of entries) {
      if (entry != null) strings.push(String(entry).trim());
    }
  }
...
function selectedBuildIdsFromArgs(args: Partial<CliArgs> = {}): Set<string> {
  const explicit = args.build;
  const buildValues: readonly unknown[] = Array.isArray(explicit) ? explicit : [explicit];
  const positional = toArray(args._)
    .filter((value) => value.startsWith("build:"))
    .map((value) => value.slice(6));
  return new Set([...buildValues, ...positional].map((value) => String(value ?? "").trim()).filter(Boolean));
}
...
export async function loadCuratedBuildContext(options: Record<string, string | undefined> = {}) {
  const paths = resolveCuratedBuildPaths(options);
...
async function materializeCuratedBuilds({
  ...
  for (const entry of toArray(state.build_plane?.curated_builds)) {
    ...
    const projectEntry = projectsById.get((entry.source_project ?? "")) ?? null;
  ...
  async function walk(currentPath: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && entry.name.endsWith(".build.sweetspot.json")) {
        const manifest = await maybeReadJson<BuildManifest>(fullPath);
        files.push({
...
async function collectPublishBundlesByBuildId(
  ...
  for (const bundle of toArray(publishIndex?.bundles)) {
    ...
    const report = await maybeReadJson<PublishReport>(path.join(bundleDir, "publish-report.json"));
    const manifestEntry = manifestsByBuildId.get(buildId) ?? null;
    const projectRoot = projectsById.get((manifestEntry?.manifest?.source?.project ?? ""))?.root ?? null;
...
function summarizeLeakHotspots(report: PublishReport | null | undefined, sourceRoots: SourceRoot[]) {
  ...
  return [...counts.values()]
    .sort((left, right) => right.count - left.count || (left.actual_path ?? "").localeCompare((right.actual_path ?? "")))
    .slice(0, 12);
}
