...
async function scanClaudeFile(file, onMsg) {
...
function readSlice(fp, start, len) {
...
const codexRun = [];   // sessions since --codex-since  -> agent rows
const codexWeek = [];  // sessions in the 7d window     -> denominator
(function walk(dir) {
...
const money = (n) => n == null ? 'unavailable' : `$${n.toFixed(2)}`;
const pct = (cost, denom, label) =>
