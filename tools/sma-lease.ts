 ...
 * Usage: `node tools/sma-lease.ts list --resource-kind brick --json`
 */
/**
...
function usage() {
  console.log(`Usage:
  sma-lease.ts acquire        --resource-kind <kind> --resource <id> [--agent <id>] --intent "..."
                               [--ttl <seconds>] [--project <id>] [--brick <id>] [--session <id>]
                               [--task <id>] [--model <name>] [--actor-kind <kind>]
                               [--rationale "..."] [--linked-backlog <id>]... [--auto-context]
  ...
  sma-lease.ts list           [--resource-kind <kind>] [--resource <id>] [--agent <id>]
                               [--include-expired] [--json]
  sma-lease.ts status         --resource-kind <kind> --resource <id> [--json]
  sma-lease.ts expire
  sma-lease.ts run            --resource-kind <kind> --resource <id> [--agent <id>] --intent "..."
                               [--ttl <seconds>] [--auto-context] [--project <id>] [--brick <id>]
                               [--renew-every <seconds>] -- <command...>
...
function doRenew(leaseId: string, ttl: number, owner: string | null, maxWaitMs = LOCK_MAX_WAIT_MS): Lease {
  return mutateRegistry((reg) => {
    ...
    if (!lease) {
      const err = (new Error(`lease not found (or already expired): ${leaseId}`)) as Error & { code?: number };
      err.code = 12;
      throw err;
    }
...
function doRelease(leaseId: string, owner: string | null): Lease {
  return mutateRegistry((reg) => {
    ...
    if (idx < 0) {
      const err = (new Error(`lease not found: ${leaseId}`)) as Error & { code?: number };
      err.code = 12;
      throw err;
    }
