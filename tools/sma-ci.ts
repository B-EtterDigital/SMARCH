 ...
 * Usage: `node tools/sma-ci.ts --help`
 */
...
Usage:
  node tools/sma-ci.ts [--root <dir>] [--registry <path>] [--wiki <path>]
...
function runContextCheck(options: Record<string, any>) {
  ...
  for (const projectId of projects) {
    ...
    const res = spawnSync(process.execPath, [path.join(smaRoot, "tools", "sma-context-check.ts"), ...args], {
      stdio: "inherit"
    });
    if (res.status === 3) {
      totalMissing += 1;
...
function runConflictCheck(options: Record<string, any>) {
  ...
  for (const projectId of projects) {
    const res = spawnSync(process.execPath, [
      path.join(smaRoot, "tools", "sma-conflict.ts"),
      "check",
      "--project",
      projectId,
      "--json"
...
function runDirtyClaimCheck(options: Record<string, any>) {
  ...
  const res = spawnSync(process.execPath, [
    path.join(smaRoot, "tools", "sma-controller-snapshot.ts"),
    ...snapshotArgs
  ], {
    encoding: "utf8"
  });
...
function main() {
  ...
  }, "sma-scan.ts", ["--root", options.root, "--out", options.registry]);
  run("validate", "sma-validate.ts", ["--registry", options.registry]);
  run("security", "sma-security-gate.ts", ["--root", options.root]);
  ...
  run("rule-gate", "sma-rule-gate.ts", ["--all", "--report", "security/rule-gate.generated.json"]);
  run("scope-drift", "sma-scope-drift.ts", ["--all", "--report", "security/scope-drift.generated.json"]);
  ...
  run("license-gate", "sma-license-gate.ts", ["--gate"]);
  run("provenance-verify", "sma-provenance-verify.ts", ["--gate"]);
  run("wiki", "sma-wiki.ts", ["--registry", options.registry, "--out", options.wiki]);
