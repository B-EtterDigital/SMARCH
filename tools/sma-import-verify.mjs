...
function normalizeMaybeAbsolutePath(root, value) {
...
function uniqStrings(values) {
  return uniq(safeArray(values).map((value) => String(value || "").trim()).filter(Boolean));
}
...
function requireObject(recorder, value, code, message, detail = {}) {
  if (!isObject(value)) {
...
function requireArray(recorder, value, code, message, detail = {}) {
...
function requireString(recorder, value, code, message, detail = {}) {
...
function requireBoolean(recorder, value, code, message, detail = {}) {
...
function validateImportId(recorder, importId, code, detail = {}) {
...
function validateHash(recorder, value, code, message, detail = {}, { required = false } = {}) {
  if (!isNonEmptyString(value)) {
...
function validateSemver(recorder, value, code, detail = {}) {
...
function validateArtifactType(recorder, value, code, detail = {}) {
...
function collectContractEnvBindings(importRecord) {
  const collected = [];
  const contracts = isObject(importRecord.contracts) ? importRecord.contracts : {};
  const importId = importRecord.import_id;
...
function mergeEnvBinding(envMap, binding, placementId = "") {
  ...
  const current = envMap.get(name) || {
    ...
    surfaces: new Set(),
    import_ids: new Set(),
    placement_ids: new Set(),
    aliases: new Set()
  };
  current.required = current.required || Boolean(binding.required);
...
function mergeEnvMap(targetMap, sourceMap) {
...
function summarizeEnvMap(envMap, processEnv) {
  const variables = [];
...
function validateImportsDocument(doc, recorder) {
  const summary = {
    total_imports: 0,
    by_status: {},
    by_artifact_type: {},
  ...
  if (doc.schema !== "smarch.imports.v0") {
    recorder.warn("imports.schema", `Unexpected imports schema "${doc.schema || "missing"}"`, { expected: "smarch.imports.v0" });
  } else {
  ...
  for (let i = 0; i < doc.imports.length; i += 1) {
    ...
    if (importedAtOk && !isDateTime(entry.imported_at)) {
      recorder.warn("imports.entry.imported_at.invalid", `Invalid imported_at "${entry.imported_at}"`, detail);
    }
    ...
    summary.imports.push({
      ...
      artifact_type: artifactTypeOk ? entry.artifact_type : "unknown",
      artifact_id: artifactIdOk ? entry.artifact_id : null,
      status: entry.status,
      imported_at: importedAtOk ? entry.imported_at : null,
      target_paths: safeArray(entry.target_paths).map((value) => normalizePath(value))
    });
  }

  recorder.pass("imports.parsed", `Parsed ${summary.total_imports} import record(s)`, { count: summary.total_imports });
  return summary;
}

function validateLockedArtifacts(recorder, artifacts, expectedArtifactType, codePrefix) {
  ...
  for (let i = 0; i < artifacts.length; i += 1) {
    ...
    if (artifact.verification_status && !verificationStatuses.has(artifact.verification_status)) {
    ...
    summary.push({
      ...
      release_hash: artifact.release_hash || null,
      channel: artifact.channel || null,
      source_project: artifact.source_project || null,
      trust_tier: artifact.trust_tier || null,
      verification_status: artifact.verification_status || null,
      local_overrides: Number(artifact.local_overrides || 0)
    });
  }
  return {
...
function validateBuildLock(doc, recorder, targetRoot) {
  ...
  if (doc.schema_version !== "1.0.0") {
    recorder.fail("build_lock.schema_version", `Expected schema_version 1.0.0 but found "${doc.schema_version || "missing"}"`);
  } else {
  ...
  if (isObject(doc.lock)) {
    ...
    if (!["exact", "channel", "mixed"].includes(doc.lock.mode)) {
      recorder.fail("build_lock.mode", `Unsupported lock mode "${doc.lock.mode || "missing"}"`);
    }
    ...
    const expectedOrigin = process.env.SMA_REGISTRY_ORIGIN || null;
    ...
    if (requireArray(recorder, doc.frozen_dependency_graph.nodes, "build_lock.graph.nodes", "frozen_dependency_graph.nodes must be an array")) {
      for (let i = 0; i < doc.frozen_dependency_graph.nodes.length; i += 1) {
        ...
        summary.graph_nodes.push({
          ...
          artifact_type: node.artifact_type || null,
          artifact_id: node.artifact_id || null,
          release_version: node.release_version || null,
          release_hash: node.release_hash || null
        });
    ...
    if (requireArray(recorder, doc.frozen_dependency_graph.edges, "build_lock.graph.edges", "frozen_dependency_graph.edges must be an array")) {
      ...
      for (let i = 0; i < doc.frozen_dependency_graph.edges.length; i += 1) {
        ...
        if (!graphRelations.has(edge.relation)) {
          recorder.fail("build_lock.graph.edge.relation", `Unsupported edge relation "${edge.relation || "missing"}"`, detail);
        }
  ...
  if (isObject(doc.trust_policy)) {
    ...
    for (const status of doc.trust_policy.allowed_release_statuses) {
      if (!releaseStatuses.has(status)) recorder.fail("build_lock.trust.allowed_release_statuses.invalid", `Unsupported release status "${status}"`);
    }
    if (doc.trust_policy.minimum_verification_status && !verificationStatuses.has(doc.trust_policy.minimum_verification_status)) {
  ...
  if (isObject(doc.verification_policy)) {
    ...
    if (!requiredCheckStatuses.has(doc.verification_policy.required_check_status)) {
      recorder.fail("build_lock.verify.required_check_status", `Unsupported required_check_status "${doc.verification_policy.required_check_status || "missing"}"`);
    }
  }
...
function validatePlacementImports(recorder, imports) {
  const seenImportIds = new Set();
  const summary = [];
  for (let i = 0; i < imports.length; i += 1) {
    ...
    if (entry.imported_at && !isDateTime(entry.imported_at)) {
      recorder.warn("placements.imports.imported_at", `Invalid placements imported_at "${entry.imported_at}"`, detail);
    }
    if (entry.status && !placementImportStatuses.has(entry.status)) {
    ...
    summary.push({
      ...
      release_hash: entry.release_hash || null,
...
function validatePlacements(doc, recorder, targetRoot) {
  const summary = {
    imports: [],
    import_ids: [],
    placements: [],
  ...
  if (doc.schema_version !== "1.0.0") {
    recorder.fail("placements.schema_version", `Expected schema_version 1.0.0 but found "${doc.schema_version || "missing"}"`);
  } else {
  ...
  for (let i = 0; i < doc.placements.length; i += 1) {
    ...
    if (!placementKinds.has(entry.kind)) {
      recorder.fail("placements.entry.kind", `Unsupported placement kind "${entry.kind || "missing"}"`, detail);
    }
    ...
    for (const envBinding of safeArray(entry.env_bindings)) {
      ...
      if (!envSurfaces.has(envBinding.surface)) {
        recorder.warn("placements.entry.env_bindings.surface", `Unexpected env binding surface "${envBinding.surface || "missing"}"`, {
          ...detail,
      ...
      envBindings.push({
        ...
        surface: envBinding.surface || "server",
    ...
    summary.placements.push({
      ...
      adapter_points_pending: safeArray(entry.adapter_points).filter((point) => point?.status !== "bound").length,
      env_binding_count: envBindings.length
    });
  }

  recorder.pass("placements.parsed", `Parsed ${summary.placements.length} placement record(s)`, {
...
function validateJournal(events, recorder) {
  const summary = {
    total_events: 0,
    by_result: {},
    by_event_type: {},
  ...
  for (const { line_number, record } of events) {
    ...
    if (!isDateTime(record.created_at)) recorder.warn("journal.created_at.invalid", `Invalid journal created_at "${record.created_at}"`, detail);
    ...
    if (record.result && !updateResults.has(record.result)) {
      recorder.warn("journal.result", `Unexpected journal result "${record.result}"`, detail);
    }

    summary.total_events += 1;
    incrementCounter(summary.by_result, record.result || "unknown");
    incrementCounter(summary.by_event_type, record.event_type || "unknown");
  ...
  else recorder.pass("journal.parsed", `Parsed ${summary.total_events} update journal event(s)`, { count: summary.total_events });

  return summary;
}

async function verifyPlacementTargets(placementsSummary, targetRoot, recorder) {
  const health = {
  ...
  for (const placement of placementsSummary.placements) {
    ...
    perImport.local_overrides += Number(placement.local_overrides || 0);
    perImport.pending_adapter_points += Number(placement.adapter_points_pending || 0);
    if (placement.ownership_replaceable === false) perImport.replaceable_false += 1;
    ...
    health.local_overrides = (health.local_overrides || 0) + Number(placement.local_overrides || 0);
    health.pending_adapter_points = (health.pending_adapter_points || 0) + Number(placement.adapter_points_pending || 0);
    health.replaceable_false = (health.replaceable_false || 0) + (placement.ownership_replaceable === false ? 1 : 0);
    health.env_binding_count = (health.env_binding_count || 0) + Number(placement.env_binding_count || 0);
...
function buildImportHealth(importsSummary, placementsHealth, journalSummary) {
...
function compareReleaseAlignment(left, right) {
  const comparable = Boolean(isObject(left) && isObject(right));
  if (!comparable) {
    return {
      comparable: false,
...
function summarizeLockedArtifactEvidence(artifacts, minimumVerificationStatus) {
  const byVerificationStatus = {};
  const byTrustTier = {};
  const byChannel = {};
  const bySourceProject = {};
  ...
  for (const artifact of safeArray(artifacts)) {
    incrementCounter(byVerificationStatus, artifact.verification_status || "unknown");
    incrementCounter(byTrustTier, artifact.trust_tier || "unknown");
    incrementCounter(byChannel, artifact.channel || "unknown");
    incrementCounter(bySourceProject, artifact.source_project || "unknown");
    if (Number(artifact.local_overrides || 0) > 0) localOverrideImports += 1;
  ...
  return {
    total: safeArray(artifacts).length,
...
function summarizePlacementIntegrity(placementsSummary, placementsHealth) {
  return {
    ...
    imports_with_drift: Object.values(placementsHealth.by_import).filter((entry) => entry.hash_mismatches > 0).length,
    imports_with_missing_targets: Object.values(placementsHealth.by_import).filter((entry) => entry.missing_targets > 0).length,
    imports_with_pending_adapter_points: Object.values(placementsHealth.by_import).filter((entry) => entry.pending_adapter_points > 0).length,
    by_import: Object.entries(placementsHealth.by_import)
      .map(([importId, entry]) => ({
        import_id: importId,
...
function summarizeBuildResolution({ importsSummary, buildLockSummary, placementsSummary, buildLockDoc }) {
  const selectedBuilds = safeArray(buildLockSummary.selected_builds.artifacts);
  const resolvedBricks = safeArray(buildLockSummary.resolved_bricks.artifacts);
  const graphNodes = safeArray(buildLockSummary.graph_nodes);
  const graphEdges = safeArray(buildLockSummary.graph_edge_records);
  const graphNodeIds = new Set(graphNodes.map((node) => node.node_id).filter(Boolean));
  const importsById = new Map(safeArray(importsSummary.imports).map((entry) => [entry.import_id, entry]));
  const placementImportsById = new Map(safeArray(placementsSummary.imports).map((entry) => [entry.import_id, entry]));
  const minimumVerificationStatus = buildLockDoc?.trust_policy?.minimum_verification_status || null;
  ...
  const selectedBuildsWithoutResolvedBricks = selectedBuilds
    .filter((entry) => !(outgoing.get(entry.import_id) || []).some((edge) => resolvedBrickIds.has(edge.to)))
    .map((entry) => entry.import_id);
  const resolvedBricksWithoutParentBuild = resolvedBricks
    .filter((entry) => !(incoming.get(entry.import_id) || []).some((edge) => selectedBuildIds.has(edge.from)))
    .map((entry) => entry.import_id);
  ...
  return {
    ...
    coverage: {
      imports_in_lock: safeArray(importsSummary.imports).filter((entry) => selectedBuildIds.has(entry.import_id) || resolvedBrickIds.has(entry.import_id)).length,
      imports_missing_from_lock: safeArray(importsSummary.imports).filter((entry) => !selectedBuildIds.has(entry.import_id) && !resolvedBrickIds.has(entry.import_id)).map((entry) => entry.import_id),
      placement_imports_in_lock: safeArray(placementsSummary.imports).filter((entry) => selectedBuildIds.has(entry.import_id) || resolvedBrickIds.has(entry.import_id)).length,
      placement_imports_missing_from_lock: safeArray(placementsSummary.imports).filter((entry) => !selectedBuildIds.has(entry.import_id) && !resolvedBrickIds.has(entry.import_id)).map((entry) => entry.import_id)
    },
    ...
    trust_signals: {
      ...
      allow_local_overrides: Boolean(buildLockDoc?.trust_policy?.allow_local_overrides),
      require_contract_hashes: Boolean(buildLockDoc?.trust_policy?.require_contract_hashes),
    ...
    imports: safeArray(importsSummary.imports)
      .map((entry) => ({
        import_id: entry.import_id,
        artifact_type: entry.artifact_type,
        artifact_id: entry.artifact_id,
...
function findRegistrySnapshotValues(importsDoc, buildLockDoc, placementsDoc, journalEvents) {
  return uniq([
    ...safeArray(importsDoc?.imports).map((entry) => entry?.source_registry?.sha256),
    buildLockDoc?.lock?.registry_snapshot_sha,
    placementsDoc?.map?.registry_snapshot_sha,
    ...journalEvents.map(({ record }) => record?.registry_snapshot_sha)
  ]);
}

function finalizeStatus(counts) {
...
async function main() {
  ...
  let importsDoc = null;
  let buildLockDoc = null;
  let placementsDoc = null;
  let journalEvents = [];
  ...
  for (const [key, file] of Object.entries(files)) {
    ...
    try {
      ...
      recorder.fail("file.parse", `Failed to parse ${file.path}: ${error.message}`, { file: key, path: file.path });
    }
  }

  const importsSummary = files.imports.parsed ? validateImportsDocument(importsDoc, recorder) : {
    total_imports: 0,
    by_status: {},
    by_artifact_type: {},
    import_ids: [],
  ...
  const buildLockSummary = files.build_lock.parsed ? validateBuildLock(buildLockDoc, recorder, targetRoot) : {
    selected_builds: { import_ids: [], artifacts: [] },
    resolved_bricks: { import_ids: [], artifacts: [] },
    graph_node_ids: new Set(),
    graph_nodes: [],
  ...
  const placementsSummary = files.placements.parsed ? validatePlacements(placementsDoc, recorder, targetRoot) : {
    imports: [],
    import_ids: [],
    placements: [],
    env_bindings: new Map(),
  ...
  const journalSummary = files.update_journal.parsed ? validateJournal(journalEvents, recorder) : {
    total_events: 0,
    by_result: {},
    by_event_type: {},
    latest_created_at: null,
  ...
  const lockImportIds = new Set([
    ...safeArray(buildLockSummary.selected_builds.import_ids),
    ...safeArray(buildLockSummary.resolved_bricks.import_ids)
  ]);
  ...
  const failOnMissingEnv = Boolean(buildLockDoc?.verification_policy?.fail_on_missing_env);
