 ...
 * Usage: `node tools/sma-codex.ts --prompt "Reply with PONG"`
 */
/**
 * Thin CLI for the codex-runner. Useful for ad-hoc shell tests.
 *
 *   node tools/sma-codex.ts --prompt "Summarize the SMA framework in one sentence"
 *   node tools/sma-codex.ts --prompt-file ./prompt.txt --schema ./schema.json
 *   echo "Reply with PONG" | node tools/sma-codex.ts --stdin
 */
