...
function readControllerSnapshot() {
  const controllerArgs = [
    resolve(SMA_ROOT, 'tools/sma-controller-snapshot.mjs'),
    '--json',
    '--module-graphs',
...
function render({ gen3, liveLeases, controller }) {
  const ctx = gen3?.context_coverage ?? { projects_with_logs: 0, total_bricks_with_context: 0, total_context_events: 0, by_project: {} };
...
function readConflictSummary() {
  try {
    const raw = execFileSync(process.execPath, [
      resolve(SMA_ROOT, 'tools/sma-conflict.mjs'),
      'summary',
      '--all',
      '--json',
      '--limit',
...
function readParallelPreflight() {
  try {
    const projectFilters = asArray(args.project);
    const preflightArgs = [
      resolve(SMA_ROOT, 'tools/sma-parallel-preflight.mjs'),
      '--auto-limit',
      '--max-agents',
