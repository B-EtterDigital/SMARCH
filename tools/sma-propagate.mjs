 ...
 *      existing tools/sma-update-plan.mjs to produce a per-target update plan.
 ...
 *
 * Use:
 *   node tools/sma-propagate.mjs --source-brick <id> [--release <path>] [--apply]
 *   node tools/sma-propagate.mjs --since <git-sha> --source-project acme-desktop
 */
...
if (args.help || (!args.sourceBrick && !args.since)) {
  console.log(`Usage:
  sma-propagate.mjs --source-brick <id> [--release <path>] [--apply] [--json]
  sma-propagate.mjs --since <git-sha> --source-project <id> [--release <path>]
...
if (!existsSync(DEPENDENTS_INDEX)) {
  console.error(`Dependents index missing — run: node tools/sma-dependents-index.mjs --write`);
  exit(1);
}
...
for (const brickId of targetBricks) {
  ...
  for (const l of links) {
    ...
    if (l.evidence_kind === 'import-lock') {
      ...
      const planArgs = [smaPath('tools/sma-update-plan.mjs'), '--target', l.target_root];
