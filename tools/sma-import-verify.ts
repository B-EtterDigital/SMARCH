...
function uniqStrings(values): string[] {
  return uniq(safeArray(values).map((value) => String(value || "").trim()).filter(Boolean)) as string[];
}
...
function summarizePlacementIntegrity(placementsSummary, placementsHealth: Record<string, any>) {
  const byImport: Record<string, any> = placementsHealth.by_import || {};
  return {
...
async function main() {
  ...
  const importIds = new Set<string>(importsSummary.import_ids as string[]);
  ...
  const placementImportIds = new Set<string>(placementsSummary.import_ids as string[]);
