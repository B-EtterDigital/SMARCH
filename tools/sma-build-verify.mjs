...
Usage:
  node tools/sma-build-verify.mjs [options]
  node tools/sma-build-verify.mjs --manifest builds/acme-studio/ai-image-generation.build.sweetspot.json
...
Examples:
  node tools/sma-build-verify.mjs --stdout
  node tools/sma-build-verify.mjs --manifest builds/<project>/<build>.build.sweetspot.json
  node tools/sma-build-verify.mjs --root builds --out builds/build-verify.generated.json
`;
...
function createRecorder(maxChecks) {
  ...
  function push(level, code, message, options = {}) {
    counts[level] += 1;
    const check = {
      level,
      code,
...
async function verifyBuildManifest(document, filePath, options) {
...
function buildSignal(area, checks, options = {}) {
